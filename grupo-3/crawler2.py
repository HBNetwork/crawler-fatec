...
def procurar_id_curso_por_nome(df, curso):
    ...
    return x['id'].to_string(index=False) if len(x['id']) > 0 else 0
    ...
    for i in tqdm(range(len(dfFatec))):
        ...
        for c in range(len(listaDeCursos)):
            ...
            if "Selecione..." not in listaDeCursos[c]["curso"]:
                ...
                # OTIMIZO O DISPARO PARA A PÁGINA DE RESULTADO
                classificacao_geral_vest_fatec.get(
                    f'{classificacao_geral_vest_fatec.current_url}?codfatec={id_}&codescolacurso={idC}&o=1'
                )     
                time.sleep(5)
                ...
                try:
                    ...
                    resultado80 = tabela_.find_elements(
                        By.CSS_SELECTOR, "tbody tr:nth-child(-n + 80)"
                    )
                    # print(resultado80)
                ...
                lista_info_vestibular_fatec["cod_resultado_fatec"] = 0
                lista_info_vestibular_fatec["codcursofatec"] = procurar_id_curso_por_nome(
                    dfCursos, curso_periodo(listaDeCursos[c]["curso"])[0]
                )  # idC # Não é o código do Curso. Precisa fazer
                lista_info_vestibular_fatec["cod_curso"] = 0
                lista_info_vestibular_fatec["nome_curso"] = (
                    curso_periodo(listaDeCursos[c]["curso"])[0]
                ).strip()  # curso
                lista_info_vestibular_fatec["codfatec"] = id_
                lista_info_vestibular_fatec["cod_instituicao"] = 0
                lista_info_vestibular_fatec["instituicao"] = extrai_nome_fatec(fatec_.strip())
                ...
                lista_info_vestibular_fatec["qtde_vagas"] = 0
                lista_info_vestibular_fatec["qtde_inscrito"] = 0
                lista_info_vestibular_fatec["demanda"] = 0
                ...
                # RETORNO 1 VEZ PARA RECOMEÇAR O PASSO A PASSO                
